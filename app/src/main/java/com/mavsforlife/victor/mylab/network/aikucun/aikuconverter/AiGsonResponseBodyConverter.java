...
public class AiGsonResponseBodyConverter<T> implements Converter<ResponseBody, T> {
    private static final Charset UTF_8 = Charset.forName("UTF-8");
    private final Gson mGson;
    private final TypeAdapter<T> adapter;

    public AiGsonResponseBodyConverter(Gson gson, TypeAdapter<T> adapter) {
        mGson = gson;
    ...
    public T convert(ResponseBody value) throws IOException {
        ...
        AikucuResult re = mGson.fromJson(response, AikucuResult.class);
        //关注的重点，自定义响应码中非0的情况，一律抛出ApiException异常。
        //这样，我们就成功的将该异常交给onError()去处理了。
        if (re.getCode() != 0) {
            value.close();
            throw new AiApiException(re.getCode(), re.getMessage());
        }
